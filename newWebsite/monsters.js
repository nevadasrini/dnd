...
//Contains monster info in json format
var data = [
  {
    "name": "Aboleth",
...
/**
 * searchName(monName) -> object literal
  Returns the monsters with the provided name or similar names
  */
function searchName(monName, inputList){
  var monList = []; // contains the desired monsters
  monName = monName.toUpperCase().trim(); // case check
  var nonsense = new RegExp('\W+');
  if (monName.length === 0 || nonsense.test(monName))
  {
    return inputList;
  }
  //Search for name
    for (var i =0; i< inputList.length; i++){
        if (inputList[i]["name"].toUpperCase().trim().includes(monName)){
            monList.push(inputList[i]);
        }
    }
    //Sort by relvance
    monList.sort(function(mon1, mon2){
        var loc1 = mon1["name"].toUpperCase().trim().indexOf(monName);
        var loc2 = mon2["name"].toUpperCase().trim().indexOf(monName);
        return loc1 - loc2;
    })
...
/**
 * @param {*} armClass an int specifying the armor class you want monsters to have
 * @param {*} inputList the list of monsters you are searching through
 * @return a list of monsters with the provided armor class; empty list if no such monsters exist
 */
var searchArmor = function(armClass, inputList)
{
    if (armClass == 0){
        return inputList;
    }
  return inputList.filter(function(monster) {
    return monster["Armor Class"].trim().slice(0, monster["Armor Class"].indexOf(" ")) == armClass;
  })
}
...
/**
 * 
 * @param {*} chalRat the desired challenge rating in a monster
 * @param {*} inputList the list that monsters are being filtered from
 * @return a list of monsters with the provided challenge rating OR LESS
 */
var searchChalRat = function(chalRat, inputList)
{
  return inputList.filter(function(monster) {
    return monster["Challenge"].trim().slice(0, monster["Challenge"].indexOf(" ")) >= chalRat;
  })
}


/**
 * @param {*} hp an int specifying the hp you want monsters to have
 * @param {*} inputList the list of monsters you are searching through
 * @return a list of monsters with the provided hp; empty list if no such monsters exist
 */
var searchHp = function(hp, inputList)
{
    if (hp == 0){
        return inputList;
    }
  return inputList.filter(function(monster) {
    return monster["Hit Points"].trim() == hp;
  })
}


/**
 * @param {*} str an int specifying the str you want monsters to have
 * @param {*} inputList the list of monsters you are searching through
 * @return a list of monsters with the provided str; empty list if no such monsters exist
 */
var searchStr = function (str, inputList)
{
    if (str == 0){
        return inputList;
    }
  return inputList.filter(function(monster) {
    return monster["STR"].trim() == str;
  })
}


/**
 * @param {*} dex an int specifying the dex you want monsters to have
 * @param {*} inputList the list of monsters you are searching through
 * @return a list of monsters with the provided dex; empty list if no such monsters exist
 */
var searchDex= function (dex, inputList)
{
    if (dex == 0){
        return inputList;
    }
  return inputList.filter(function(monster) {
    return monster["DEX"].trim() == dex;
  })
}


/**
 * @param {*} con an int specifying the con you want monsters to have
 * @param {*} inputList the list of monsters you are searching through
 * @return a list of monsters with the provided con; empty list if no such monsters exist
 */
var searchCon= function (con, inputList)
{
    if (con == 0){
        return inputList;
    }
  return inputList.filter(function(monster) {
    return monster["CON"].trim() == con;
  })
}


/**
 * @param {*} int an int specifying the int you want monsters to have
 * @param {*} inputList the list of monsters you are searching through
 * @return a list of monsters with the provided int; empty list if no such monsters exist
 */
var searchInt= function (int, inputList)
{
    if (int == 0){
        return inputList;
    }
  return inputList.filter(function(monster) {
    return monster["INT"].trim() == int;
  })
}


/**
 * @param {*} wis an wis specifying the wis you want monsters to have
 * @param {*} inputList the list of monsters you are searching through
 * @return a list of monsters with the provided wis; empty list if no such monsters exist
 */
var searchWis = function (wis, inputList)
{
    if (wis == 0){
        return inputList;
    }
  return inputList.filter(function(monster) {
    return monster["WIS"].trim() == wis;
  })
}
...
/**
 * @param {*} int an cha specifying the cha you want monsters to have
 * @param {*} inputList the list of monsters you are searching through
 * @return a list of monsters with the provided cha; empty list if no such monsters exist
 */
var searchCha = function (cha, inputList)
{
    if (cha == 0){
        return inputList;
    }
  return inputList.filter(function(monster) {
    return monster["CHA"].trim() == cha;
  })
}
...
/**
 * @param {*} int an speed specifying the speed you want monsters to have
 * @param {*} inputList the list of monsters you are searching through
 * @return a list of monsters with the provided speed; empty list if no such monsters exist
 */
var searchSpeed = function (speed, inputList)
{
    if (speed == 0){
        return inputList;
    }
  return inputList.filter(function(monster) {
    return monster["Speed"].trim().slice(0, monster["Speed"].indexOf(" ")) == speed;
  })
}
...
var filterMons = function () {
    var resultsList = data.slice();

    //Filter Accordingly
    var filtFuncs = [searchName, searchChalRat, searchArmor, searchHp, searchSpeed, searchStr, searchDex, searchInt, searchWis, searchCon, searchCha];

    var searchQueries = []; //list of inputs
    var controls = document.getElementsByTagName("input");

    for (var i = 0; i < controls.length; i++) //not including the name query
    {
        searchQueries.push(controls[i].value);
    }

    //run each input through correpsonding filter
    for (var n = 0; n < filtFuncs.length; n++)
    {
        resultsList = filtFuncs[n](searchQueries[n], resultsList);
    }
    return resultsList;
}

//template for each monster card
var cardTemp = document.getElementsByClassName("card")[0];

//Create a monster card for each search result
var loadMons = function (monstersList) {
	var currRes = document.getElementsByClassName("results")[0];
    while (currRes.children.length >0){
        currRes.removeChild(currRes.lastChild);
    }
    cardTemp.style = "display: block";
    for (var k=0; k< monstersList.length; k++)
    {
        var newCard = cardTemp.cloneNode(true);
        newCard.children[0].children[0].src = monstersList[k]["img_url"];
        newCard.children[1].children[0].textContent = monstersList[k]["name"];
        newCard.children[1].children[1].textContent = monstersList[k]["meta"];
        newCard.children[2].children[0].innerHTML= monstersList[k]["name"] + '<i class="material-icons right">close</i>';
        var statTypes = ["STR", "DEX", "CON", "INT", "WIS", "CHA"];
        var statDesc = "";
        for (var i = 0; i < 6; i++){
            statDesc += statTypes[i] + ": " + monstersList[k][statTypes[i]] + "\n";
        }
        newCard.children[2].children[1].textContent= statDesc;
        newCard.children[2].children[2].innerHTML= "<strong>TRAITS:<strong>";
        newCard.children[2].children[3].innerHTML= monstersList[k]["Traits"] || "none";
        document.getElementsByClassName("results")[0].appendChild(newCard);
    }
